...
class AppState : ObservableObject {
    ...
    func fetchUsers() {
        ref.child("users").observe(.childAdded, with: { (snapshot) in
            if let dictionary = snapshot.value as? [String: AnyObject] {
                ...
                if let image = dictionary["profileImageUrl"] as? String {
                    user.profileImageUrl = image
                }
                user.id = snapshot.key
                if self.users.filter({ $0.email == user.email }).isEmpty {
    ...
    func createUser(user: UserData) {
        ...
        ref.child("users").child(uid).setValue(param) { (error, ref) in
            if let error = error {
                print(error)
            }
        }
    ...
    //MARK:- Update profile picture
     func updateProfileImage(url: String) {
        let ref = self.ref.child("users").child(currentUser?.uid ?? "uid")
        ref.updateChildValues(["profileImageUrl": url])
    }
